...
$query = "SELECT COUNT(id) FROM classroomGuide_ps14Videos;";
$result = mysqli_query($conn, $query);
$videosCount = mysqli_fetch_assoc($result);
$videosCount = $videosCount["COUNT(id)"];

$query2 = "SELECT path from classroomGuide_ps14Videos;";
$result2 = mysqli_query($conn,$query2);
$pathList = array();
while ($row = $result2->fetch_array())
	$pathList[] = $row[0];

$query2 = "SELECT id from classroomGuide_ps14Videos;";
$result2 = mysqli_query($conn,$query2);
$pathIdList = array();
while ($row = $result2->fetch_array())
	$pathIdList[] = $row[0];

$query3 = "SELECT COUNT(id) FROM classroomGuide_ps14Rewards;";
$result3 = mysqli_query($conn, $query3);
$rewardsCount = mysqli_fetch_assoc($result3);
$rewardsCount = $rewardsCount["COUNT(id)"];

$query4 = "SELECT img from classroomGuide_ps14Rewards;";
$result4 = mysqli_query($conn,$query4);
$imgList = array();
while ($row = $result4->fetch_array())
	$imgList[] = $row[0];

$query4 = "SELECT id from classroomGuide_ps14Rewards;";
$result4 = mysqli_query($conn,$query4);
$imgIdList = array();
while ($row = $result4->fetch_array())
	$imgIdList[] = $row[0];
		...
		<div class="main">
			<div class="row">
				<div class='list_section col-xs-6 col-sm-12 col-md-12'>
						<img alt="Фотография группы ПС-14" src="img/ClassroomGuide/PS14/main.jpg" class="imgInDiv">
				</div>
				<?php
					echo <<<html
...
html;
					...
					<input type="text" style="height:50px;font-size: 2em;    border: solid;width: 100%;" name="videoLink" maxlength="250" required></input>
...
html;
				...
				if(isset($_POST['addVid'])){
						...
						$sql = "Insert into classroomGuide_ps14Videos (path) values ('$videoLink');";//формирование запроса
						...
						if($result){
							...
							.then((value) => {
								location.replace('/classroomGuide_PS14.php');
							});
							</script>
html;
						}else{
												...
												<div class='list_section col-xs-6 col-sm-11 col-md-5 videoFrame'>
														<iframe width="100%"  height="315" src="{$pathList[$i]}"
														 frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture"
															...
															<a onclick="isDelete('?VideoDel_id={$pathIdList[$i]}')" style="width:100%"  class="btn btn-outline-secondary">Удалить</a>
...
html;
						...
						if(isset($_POST['addImage'])){
							$uploaddir = './img/ClassroomGuide/PS14Rewards/';//назначение дириктории загрузки
...
html;
								...
								$sql = "Insert into classroomGuide_ps14Rewards (img) values ('$uploadfile');";//формирование запроса
								$result = mysqli_query($conn, $sql);
								if($result){
									echo <<<html
									<script>
...
html;
			...
			echo 			<<<HTML
								<div class='list_section col-xs-6 col-sm-6 col-md-4 photosBlock'>
										 ...
										 <a onclick="isDelete('?Reward_del_id={$imgIdList[$i]}')" style="width:100%"  class="btn btn-outline-secondary">Удалить</a>
							 </div>
HTML;
								}
							}else{
...
HTML;
								...
								if(!empty($_GET['VideoDel_id'])){
									...
									$sql = "Delete from classroomGuide_ps14Videos  Where id = {$id_del}";//формирование запроса
									$result = mysqli_query($conn, $sql);
									if($result){
										echo <<<html
										<script>
...
html;
								...
								if(!empty($_GET['Reward_del_id'])){
									...
									$sql = "Delete from classroomGuide_ps14Rewards  Where id = {$id_del}";//формирование запроса
									$result = mysqli_query($conn, $sql);
									if($result){
										echo <<<html
										<script>
