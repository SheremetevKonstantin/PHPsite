...
$path_to_KOS = '/Студентам/Численные и математические методы/Математические методы/КОС/';
$path_to_RP = '/Студентам/Численные и математические методы/Математические методы/РП/';
$path_to_PR = '/Студентам/Численные и математические методы/Математические методы/ПР/';
$path_to_CurrentControl = '/Студентам/Численные и математические методы/Математические методы/Текущий контроль/';

$query = "SELECT COUNT(id) FROM prList_MatMetods;";
$result = mysqli_query($conn, $query);
$prListCount = mysqli_fetch_assoc($result);
$prListCount = $prListCount["COUNT(id)"];

$query2 = "SELECT number from prList_MatMetods;";
$result2 = mysqli_query($conn,$query2);
$numberList = array();
while ($row = $result2->fetch_array())
	$numberList[] = $row[0];

$query2 = "SELECT name from prList_MatMetods;";
$result2 = mysqli_query($conn,$query2);
$namesList = array();
while ($row = $result2->fetch_array())
	$namesList[] = $row[0];

$query2 = "SELECT id from prList_MatMetods;";
$result2 = mysqli_query($conn,$query2);
$IdList = array();
while ($row = $result2->fetch_array())
	$IdList[] = $row[0];
		...
		<div class="main">
			<div class="row">
				...
				if(!empty($_GET['red_prList_name'])){
					...
					$query = "SELECT number,name FROM prList_MatMetods Where id={$id_red}";
					$result = mysqli_query($conn, $query);
					if($result){
...
html;
						...
						if(isset($_POST['deploy'])){
								$number = $_POST['number'];
								$name = $_POST['prName'];
								$sql = "Update prList_MatMetods Set number='{$number}',name='{$name}' Where id = {$id_red};";//формирование запроса
								...
								if($result){
									...
									.then((value) => {
										location.replace('/disciplines_mat_metods.php');
									});
									</script>
html;
								}else{
...
html;
				...
				if(isset($_POST['addPrListBTN'])){
					$number = $_POST['number'];
					$name = $_POST['prName'];
					$sql = "Insert Into prList_MatMetods (number,name) Values ('{$number}','$name');";//формирование запроса
					$result = mysqli_query($conn, $sql);
					if($result){
...
html;
				...
				if(!empty($_GET['del_prList_name'])){
					...
					$sql = "Delete from prList_MatMetods  Where id = {$id_del}";//формирование запроса
					$result = mysqli_query($conn, $sql);
					if($result){
						echo <<<html
						<script>
					...
					<tbody style="border:double">
						...
						if($_SESSION['login'] == 'admin'){
							...
							<tr style="width: 100%;">
								...
								<td class="text_in_tableRow widthTenPercent"><a href="disciplines_mat_metods.php?red_prList_name={$IdList[$i]}">Редактировать</a></td>
				...
				<table class="table table-hover">
					<thead>
						<tr>
							...
							if($_SESSION['login'] == 'admin'){
							echo <<<html
							<th scope="col" class="h2 tableHeader" style="text-align:center;" colspan="3">Практические работы</th>
html;
							}else {
							echo <<<html
							<th scope="col" class="h2 tableHeader" style="text-align:center;" colspan="2">Практические работы</th>
html;
							}
							 ?>
						</tr>
